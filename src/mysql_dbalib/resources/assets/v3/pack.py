#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Самораспаковывающийся исходник проекта «СтройМатериалы»."""
...
FILES = {
    ...
    # ============================================================
    'README.md': r'''# СтройМатериалы: демонстрационный экзамен, вариант 3
...
- фото товаров, например `PMEZMH.jpg`, `BPV4MM.jpg`, в `app\assets\products`
...
- `database/stroymaterialy.db` - создается автоматически при запуске в SQLite-режиме.
...
- [x] Прочитать текст задания и приложения варианта 3.
...
- [x] Добавить поиск, сортировку и фильтр по производителю для менеджера и администратора.
...
''',
    ...
    # ============================================================
    'app/__init__.py': r'''"""Приложение ООО «СтройМатериалы» для демонстрационного экзамена."""
''',
    # ============================================================
...
DATABASE_PATH = DATABASE_DIR / "stroymaterialy.db"
...
DB_NAME = os.environ.get("DB_NAME", "stroymaterialy")
...
COLOR_ACCENT = "#B8860B"
COLOR_DISCOUNT = "#F4A460"
COLOR_OUT_OF_STOCK = "#BDEBFF"
COLOR_PANEL = "#FFF7DE"
...
def _prepare_product_image(value, article: str, source_dir: Path, report: list[str]) -> str | None:
    ...
    if not source.exists():
        # В исходнике для 3XBOTN указано .jpg, а файл лежит как .jpeg.
...
from config import ALL_MANUFACTURERS, ASSETS_DIR, DB_ENGINE
...
class ProductRepository:
    ...
    def list_products(
        self,
        search_text: str = "",
        manufacturer_name: str = ALL_MANUFACTURERS,
        ...
        if manufacturer_name and manufacturer_name != ALL_MANUFACTURERS:
            products = [
                product
                for product in products
                if product["manufacturer_name"] == manufacturer_name
            ]
        ...
        sort_getters = {
            "Количество на складе": lambda product: product["stock_quantity"],
            "Цена": lambda product: product["price"],
            "Действующая скидка": lambda product: product["discount"],
        }
        if sort_name in sort_getters:
            products.sort(
                key=lambda product: (sort_getters[sort_name](product), product["id"]),
...
def ensure_placeholder_image() -> Path | None:
    ...
    image = Image.new("RGB", (400, 300), "#FFF7DE")
    draw = ImageDraw.Draw(image)
    draw.rectangle((8, 8, 392, 292), outline="#DAA520", width=4)
    draw.text((118, 132), "Нет изображения", fill="#B8860B")
...
class ProductListFrame(tk.Frame):
    def __init__(self, master: tk.Misc, app: Application, user: CurrentUser) -> None:
        ...
        self.manufacturer_var = tk.StringVar(value=ALL_MANUFACTURERS)
    ...
    def _build(self) -> None:
        ...
        if self.can_search:
            ...
            manufacturer_values = [ALL_MANUFACTURERS] + [
                item["name"] for item in self.lookup_repository.get_values("manufacturers")
            ]
            tk.Label(toolbar, text="Производитель", bg=COLOR_PANEL, font=(FONT_FAMILY, 10, "bold")).grid(row=0, column=1, sticky="w", padx=8, pady=(8, 2))
            ...
            manufacturer_combo.grid(row=1, column=1, padx=8, pady=(0, 8))
            ...
            for variable in (self.search_var, self.manufacturer_var, self.sort_var, self.direction_var):
    ...
    def refresh_products(self) -> None:
        ...
        try:
            products = self.repository.list_products(
                ...
                self.manufacturer_var.get() if self.can_search else ALL_MANUFACTURERS,
...
class OrderFormWindow(tk.Toplevel):
    ...
    def _build(self) -> None:
        ...
        self.items_text: tk.Text | None = None
        ...
        tk.Label(container, text="Артикул заказа", bg=COLOR_WHITE, font=(FONT_FAMILY, 10, "bold")).grid(row=row, column=0, sticky="nw", pady=6)
        self.items_text = tk.Text(container, height=5, wrap="word", font=(FONT_FAMILY, 10))
        self.items_text.grid(row=row, column=1, columnspan=2, sticky="nsew", pady=6)
        row += 1
        tk.Label(
            container,
            text="Можно указать один артикул или пары: PMEZMH, 2, BPV4MM, 2",
            bg=COLOR_WHITE,
            fg="#555555",
    ...
    def _load_order(self) -> None:
        ...
        items_text = ", ".join(
            f"{item['article']}, {item['quantity']}" for item in order["items"]
        )
        if self.items_text:
    ...
    def _save(self) -> None:
        try:
            data = {
                ...
                "items": self.repository.parse_items_text(
                    self.items_text.get("1.0", "end") if self.items_text else ""
                ),
            }
...
''',
    ...
    # ============================================================
    'database/seed.sql': r'''INSERT OR IGNORE INTO roles(name) VALUES
    ('Администратор'),
    ('Менеджер'),
...
''',
    ...
    # ============================================================
    'database/seed_sqlite.sql': r'''-- Начальные данные для демонстрационного экзамена, вариант 3.
...
INSERT INTO users(id, role_id, full_name, login, password) VALUES (1, 1, 'Ворсин Петр Евгеньевич', '94d5ous@gmail.com', 'uzWC67');
INSERT INTO users(id, role_id, full_name, login, password) VALUES (2, 1, 'Старикова Елена Павловна', 'uth4iz@mail.com', '2L6KZG');
INSERT INTO users(id, role_id, full_name, login, password) VALUES (3, 1, 'Одинцов Серафим Артёмович', 'yzls62@outlook.com', 'JlFRCZ');
INSERT INTO users(id, role_id, full_name, login, password) VALUES (4, 2, 'Степанов Михаил Артёмович', '1diph5e@tutanota.com', '8ntwUp');
INSERT INTO users(id, role_id, full_name, login, password) VALUES (5, 2, 'Ворсин Петр Евгеньевич', 'tjde7c@yahoo.com', 'YOyhfR');
INSERT INTO users(id, role_id, full_name, login, password) VALUES (6, 2, 'Старикова Елена Павловна', 'wpmrc3do@tutanota.com', 'RSbvHv');
INSERT INTO users(id, role_id, full_name, login, password) VALUES (7, 3, 'Михайлюк Анна Вячеславовна', '5d4zbu@tutanota.com', 'rwVDh9');
INSERT INTO users(id, role_id, full_name, login, password) VALUES (8, 3, 'Ситдикова Елена Анатольевна', 'ptec8ym@yahoo.com', 'LdNyos');
INSERT INTO users(id, role_id, full_name, login, password) VALUES (9, 3, 'Никифорова Весения Николаевна', '1qz4kw@mail.com', 'gynQMT');
INSERT INTO users(id, role_id, full_name, login, password) VALUES (10, 3, 'Сазонов Руслан Германович', '4np6se@mail.com', 'AtnDjr');
INSERT INTO categories(id, name) VALUES (1, 'Общестроительные материалы');
INSERT INTO categories(id, name) VALUES (2, 'Стеновые и фасадные материалы');
INSERT INTO categories(id, name) VALUES (3, 'Сухие строительные смеси и гидроизоляция');
INSERT INTO categories(id, name) VALUES (4, 'Ручной инструмент');
INSERT INTO categories(id, name) VALUES (5, 'Защита лица, глаз, головы');
INSERT INTO suppliers(id, name) VALUES (1, 'М500');
INSERT INTO suppliers(id, name) VALUES (2, 'Изостронг');
INSERT INTO suppliers(id, name) VALUES (3, 'Knauf');
INSERT INTO suppliers(id, name) VALUES (4, 'MixMaster');
INSERT INTO suppliers(id, name) VALUES (5, 'ЛСР');
INSERT INTO suppliers(id, name) VALUES (6, 'ВОЛМА');
INSERT INTO suppliers(id, name) VALUES (7, 'Vinylon');
INSERT INTO suppliers(id, name) VALUES (8, 'Павловский завод');
INSERT INTO suppliers(id, name) VALUES (9, 'Weber');
INSERT INTO suppliers(id, name) VALUES (10, 'Hesler');
INSERT INTO suppliers(id, name) VALUES (11, 'Armero');
INSERT INTO suppliers(id, name) VALUES (12, 'Wenzo Roma');
INSERT INTO suppliers(id, name) VALUES (13, 'KILIMGRIN');
INSERT INTO suppliers(id, name) VALUES (14, 'Исток');
INSERT INTO suppliers(id, name) VALUES (15, 'RUIZ');
INSERT INTO suppliers(id, name) VALUES (16, 'Husqvarna');
INSERT INTO suppliers(id, name) VALUES (17, 'Delta');
INSERT INTO manufacturers(id, name) VALUES (1, 'М500');
INSERT INTO manufacturers(id, name) VALUES (2, 'Изостронг');
INSERT INTO manufacturers(id, name) VALUES (3, 'Knauf');
INSERT INTO manufacturers(id, name) VALUES (4, 'MixMaster');
INSERT INTO manufacturers(id, name) VALUES (5, 'ЛСР');
INSERT INTO manufacturers(id, name) VALUES (6, 'ВОЛМА');
INSERT INTO manufacturers(id, name) VALUES (7, 'Vinylon');
INSERT INTO manufacturers(id, name) VALUES (8, 'Павловский завод');
INSERT INTO manufacturers(id, name) VALUES (9, 'Weber');
INSERT INTO manufacturers(id, name) VALUES (10, 'Hesler');
INSERT INTO manufacturers(id, name) VALUES (11, 'Armero');
INSERT INTO manufacturers(id, name) VALUES (12, 'Wenzo Roma');
INSERT INTO manufacturers(id, name) VALUES (13, 'KILIMGRIN');
INSERT INTO manufacturers(id, name) VALUES (14, 'Исток');
INSERT INTO manufacturers(id, name) VALUES (15, 'RUIZ');
INSERT INTO manufacturers(id, name) VALUES (16, 'Husqvarna');
INSERT INTO manufacturers(id, name) VALUES (17, 'Delta');
INSERT INTO units(id, name) VALUES (1, 'шт.');
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (1, 'PMEZMH', 'Цемент', 1, 440.0, 1, 1, 1, 8.0, 34, 'Цемент Евроцемент М500 Д0 ЦЕМ I 42,5 50 кг', 'assets/products/PMEZMH.jpg');
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (2, 'BPV4MM', 'Пленка техническая', 1, 8.0, 2, 2, 1, 8.0, 2, 'Пленка техническая полиэтиленовая Изостронг 60 мк 3 м рукав 1,5 м, пог.м', 'assets/products/BPV4MM.jpg');
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (3, 'JVL42J', 'Пленка техническая', 1, 13.0, 2, 2, 1, 4.0, 34, 'Пленка техническая полиэтиленовая Изостронг 100 мк 3 м рукав 1,5 м, пог.м', 'assets/products/JVL42J.jpg');
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (4, 'F895RB', 'Песок строительный', 1, 102.0, 3, 3, 1, 6.0, 7, 'Песок строительный 50 кг', 'assets/products/F895RB.jpg');
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (5, '3XBOTN', 'Керамзит фракция', 1, 110.0, 4, 4, 1, 5.0, 21, 'Керамзит фракция 10-20 мм 0,05 куб.м', 'assets/products/3XBOTN.jpeg');
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (6, '3L7RCZ', 'Газобетон', 1, 7400.0, 5, 5, 2, 2.0, 20, 'Газобетон ЛСР 100х250х625 мм D400', 'assets/products/3L7RCZ.jpg');
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (7, 'S72AM3', 'Пазогребневая плита', 1, 500.0, 6, 6, 2, 5.0, 35, 'Пазогребневая плита ВОЛМА Гидро 667х500х80 мм полнотелая', 'assets/products/S72AM3.jpg');
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (8, '2G3280', 'Угол наружный', 1, 795.0, 7, 7, 2, 9.0, 20, 'Угол наружный Vinylon 3050 мм серо-голубой', 'assets/products/2G3280.jpg');
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (9, 'MIO8YV', 'Кирпич', 1, 30.0, 6, 6, 2, 9.0, 31, 'Кирпич рядовой Боровичи полнотелый М150 250х120х65 мм 1NF', 'assets/products/MIO8YV.jpg');
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (10, 'UER2QD', 'Скоба для пазогребневой плиты', 1, 25.0, 3, 3, 2, 8.0, 27, 'Скоба для пазогребневой плиты Knauf С1 120х100 мм', 'assets/products/UER2QD.jpg');
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (11, 'ZR70B4', 'Кирпич', 1, 16.0, 8, 8, 2, 3.0, 0, 'Кирпич рядовой силикатный Павловский завод полнотелый М200 250х120х65 мм 1NF', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (12, 'LPDDM4', 'Штукатурка гипсовая', 1, 500.0, 3, 3, 3, 6.0, 38, 'Штукатурка гипсовая Knauf Ротбанд 30 кг', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (13, 'LQ48MW', 'Штукатурка гипсовая', 1, 462.0, 9, 9, 3, 6.0, 33, 'Штукатурка гипсовая Knauf МП-75 машинная 30 кг', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (14, 'O43COU', 'Шпаклевка', 1, 750.0, 6, 6, 3, 1.0, 16, 'Шпаклевка полимерная Weber.vetonit LR + для сухих помещений белая 20 кг', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (15, 'M26EXW', 'Клей для плитки, керамогранита и камня', 1, 340.0, 3, 3, 3, 8.0, 0, 'Клей для плитки, керамогранита и камня Крепс Усиленный серый (класс С1) 25 кг', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (16, 'K0YACK', 'Смесь цементно-песчаная', 1, 160.0, 4, 4, 3, 8.0, 19, 'Смесь цементно-песчаная (ЦПС) 300 по ТУ MixMaster Универсал 25 кг', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (17, 'ASPXSG', 'Ровнитель', 1, 711.0, 9, 9, 3, 10.0, 20, 'Ровнитель (наливной пол) финишный Weber.vetonit 4100 самовыравнивающийся высокопрочный 20 кг', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (18, 'ZKQ5FF', 'Лезвие для ножа', 1, 65.0, 10, 10, 4, 6.0, 6, 'Лезвие для ножа Hesler 18 мм прямое (10 шт.)', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (19, '4WZEOT', 'Лезвие для ножа', 1, 110.0, 11, 11, 4, 6.0, 17, 'Лезвие для ножа Armero 18 мм прямое (10 шт.)', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (20, '4JR1HN', 'Шпатель', 1, 26.0, 10, 10, 4, 6.0, 7, 'Шпатель малярный 100 мм с пластиковой ручкой', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (21, 'Z3XFSP', 'Нож строительный', 1, 63.0, 10, 10, 4, 8.0, 5, 'Нож строительный Hesler 18 мм с ломающимся лезвием пластиковый корпус', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (22, 'I6MH89', 'Валик', 1, 326.0, 12, 12, 4, 12.0, 3, 'Валик Wenzo Roma полиакрил 250 мм ворс 18 мм для красок грунтов и антисептиков на водной основе с рукояткой', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (23, '83M5ME', 'Кисть', 1, 122.0, 11, 11, 4, 9.0, 26, 'Кисть плоская смешанная щетина 100х12 мм для красок и антисептиков на водной основе', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (24, '61PGH3', 'Очки защитные', 1, 184.0, 13, 13, 5, 6.0, 25, 'Очки защитные Delta Plus KILIMANDJARO (KILIMGRIN) открытые с прозрачными линзами', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (25, 'GN6ICZ', 'Каска защитная', 1, 154.0, 14, 14, 5, 15.0, 8, 'Каска защитная Исток (КАС001О) оранжевая', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (26, 'Z3LO0U', 'Очки защитные', 1, 228.0, 15, 15, 5, 9.0, 11, 'Очки защитные Delta Plus RUIZ (RUIZ1VI) закрытые с прозрачными линзами', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (27, 'QHNOKR', 'Маска защитная', 1, 251.0, 14, 14, 5, 2.0, 22, 'Маска защитная Исток (ЩИТ001) ударопрочная и термостойкая', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (28, 'EQ6RKO', 'Подшлемник', 1, 36.0, 16, 16, 5, 17.0, 22, 'Подшлемник для каски одноразовый', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (29, '81F1WG', 'Каска защитная', 1, 1500.0, 17, 17, 5, 2.0, 13, 'Каска защитная Delta Plus BASEBALL DIAMOND V UP (DIAM5UPBCFLBS) белая', NULL);
INSERT INTO products(id, article, name, unit_id, price, supplier_id, manufacturer_id, category_id, discount, stock_quantity, description, photo_path) VALUES (30, '0YGHZ7', 'Очки защитные', 1, 700.0, 16, 16, 5, 9.0, 36, 'Очки защитные Husqvarna Clear (5449638-01) открытые с прозрачными линзами', NULL);
...
INSERT INTO order_statuses(id, name) VALUES (1, 'Завершен');
INSERT INTO order_statuses(id, name) VALUES (2, 'Новый');
INSERT INTO orders(id, pickup_point_id, client_user_id, client_name, receive_code, status_id, order_date, delivery_date) VALUES (1, 1, 7, 'Михайлюк Анна Вячеславовна', '901', 1, '2025-02-27', '2025-04-20');
INSERT INTO orders(id, pickup_point_id, client_user_id, client_name, receive_code, status_id, order_date, delivery_date) VALUES (2, 11, 8, 'Ситдикова Елена Анатольевна', '902', 1, '2024-09-28', '2025-04-21');
INSERT INTO orders(id, pickup_point_id, client_user_id, client_name, receive_code, status_id, order_date, delivery_date) VALUES (3, 2, 9, 'Никифорова Весения Николаевна', '903', 1, '2025-03-21', '2025-04-22');
INSERT INTO orders(id, pickup_point_id, client_user_id, client_name, receive_code, status_id, order_date, delivery_date) VALUES (4, 11, 10, 'Сазонов Руслан Германович', '904', 1, '2025-02-20', '2025-04-23');
INSERT INTO orders(id, pickup_point_id, client_user_id, client_name, receive_code, status_id, order_date, delivery_date) VALUES (5, 2, 7, 'Михайлюк Анна Вячеславовна', '905', 1, '2025-03-17', '2025-04-24');
INSERT INTO orders(id, pickup_point_id, client_user_id, client_name, receive_code, status_id, order_date, delivery_date) VALUES (6, 15, 8, 'Ситдикова Елена Анатольевна', '906', 1, '2025-03-01', '2025-04-25');
INSERT INTO orders(id, pickup_point_id, client_user_id, client_name, receive_code, status_id, order_date, delivery_date) VALUES (7, 3, 9, 'Никифорова Весения Николаевна', '907', 1, NULL, '2025-04-26');
INSERT INTO orders(id, pickup_point_id, client_user_id, client_name, receive_code, status_id, order_date, delivery_date) VALUES (8, 19, 10, 'Сазонов Руслан Германович', '908', 2, '2025-03-31', '2025-04-27');
INSERT INTO orders(id, pickup_point_id, client_user_id, client_name, receive_code, status_id, order_date, delivery_date) VALUES (9, 5, 9, 'Никифорова Весения Николаевна', '909', 2, '2025-04-02', '2025-04-28');
INSERT INTO orders(id, pickup_point_id, client_user_id, client_name, receive_code, status_id, order_date, delivery_date) VALUES (10, 19, 10, 'Сазонов Руслан Германович', '910', 2, '2025-04-03', '2025-04-29');
...
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (4, 2, 4, 1);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (5, 3, 5, 10);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (6, 3, 6, 10);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (7, 4, 7, 5);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (8, 4, 8, 4);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (9, 5, 9, 2);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (10, 5, 10, 2);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (11, 6, 11, 1);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (12, 6, 12, 1);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (13, 7, 13, 10);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (14, 8, 15, 5);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (15, 8, 16, 4);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (16, 9, 17, 5);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (17, 9, 18, 1);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (18, 10, 19, 5);
INSERT INTO order_items(id, order_id, product_id, quantity) VALUES (19, 10, 20, 5);
''',
    # ============================================================
...
''',
    ...
    # ============================================================
    'database/seed_mysql.sql': r'''-- Начальные данные для демонстрационного экзамена, вариант 3.
INSERT INTO roles(id, name) VALUES (1, 'Администратор');
INSERT INTO roles(id, name) VALUES (2, 'Менеджер');
INSERT INTO roles(id, name) VALUES (3, 'Авторизированный клиент');
INSERT INTO users(id, role_id, full_name, login, password) VALUES (1, 1, 'Ворсин Петр Евгеньевич', '94d5ous@gmail.com', 'uzWC67');
...
- Товар 3XBOTN: фото 3XBOTN.jpg заменено найденным файлом 3XBOTN.jpeg.
- Заказ 7: некорректная дата 30.02.2025 заменена пустым значением.
- Заказ 7: позиция O43COU8 x 10 пропущена, так как такого артикула нет в Tovar.xlsx.
''',
    # ============================================================
...
def create_login_screenshot(output_path: Path) -> None:
    ...
    draw_box(draw, (390, 95, 810, 665), "#FFFFFF", COLOR_SECONDARY)
...
def create_products_screenshot(output_path: Path, role: str, mode: str) -> None:
    ...
    if mode == "manager":
        products = repository.list_products("пленка", "Изостронг", "Цена", "DESC")
    else:
    ...
    if mode in {"manager", "admin"}:
        draw_box(draw, (35, 88, 1165, 155), COLOR_PANEL, COLOR_SECONDARY)
        draw.text((55, 102), "Поиск: пленка", font=text_font, fill="#111111")
        draw.text((275, 102), "Производитель: Изостронг", font=text_font, fill="#111111")
        draw.text((560, 102), "Сортировка: Цена DESC", font=text_font, fill="#111111")
...
def create_product_form_screenshot(output_path: Path) -> None:
    ...
    draw_box(draw, (930, 685, 1070, 728), COLOR_ACCENT, COLOR_ACCENT)
    draw.text((958, 696), "Сохранить", font=text_font, fill=COLOR_WHITE)
...
def create_orders_screenshot(output_path: Path) -> None:
    ...
    draw_box(draw, (35, 88, 1165, 145), COLOR_PANEL, COLOR_SECONDARY)
...
def create_order_form_screenshot(output_path: Path) -> None:
    ...
    draw.text((45, y + 16), "Артикул заказа", font=label_font, fill="#111111")
    items_text = ", ".join(f"{item['article']}, {item['quantity']}" for item in order["items"])
    draw_box(draw, (290, y, 1050, y + 115), "#FFFFFF", "#999999")
    draw_wrapped_text(draw, items_text, (305, y + 16), text_font, "#333333", 700)
    draw.text(
        (305, y + 82),
        "Можно указать один артикул или пары: PMEZMH, 2, BPV4MM, 2",
        font=get_font(13),
        fill="#555555",
    )
...
def generate_er_pdf(output_path: Path) -> None:
    ...
    c.setTitle("ER-диаграмма СтройМатериалы")
    c.setFont(PDF_FONT_NAME, 20)
    c.drawString(40, 795, "ER-диаграмма базы данных ООО «СтройМатериалы»")
...
def generate_flowchart_pdf(output_path: Path) -> None:
    ...
    c.drawString(40, 550, "Алгоритм работы приложения «СтройМатериалы»")
...
def create_screenshots_docx(output_path: Path, screenshots: list[Path]) -> None:
    ...
    subtitle.add_run("ООО «СтройМатериалы». Демонстрационный экзамен, вариант 3.")

    captions = [
        "Окно входа при запуске приложения",
        "Гостевой просмотр товаров без фильтрации, сортировки и поиска",
        "Интерфейс менеджера: поиск, фильтр по производителю и сортировка",
...
def main() -> None:
    ...
    create_sql_dump(PROJECT_ROOT / "database" / "stroymaterialy_dump.sql")
...
from config import ALL_MANUFACTURERS, DB_ENGINE
...
def main() -> None:
    ...
    manager = auth_repository.authenticate("1diph5e@tutanota.com", "8ntwUp")
    client = auth_repository.authenticate("5d4zbu@tutanota.com", "rwVDh9")
    ...
    filtered_products = product_repository.list_products("пленка", "Изостронг", "Цена", "DESC")
    sorted_by_stock = product_repository.list_products("", ALL_MANUFACTURERS, "Количество на складе", "ASC")
    ...
    assert order_repository.parse_items_text("PMEZMH") == [{"article": "PMEZMH", "quantity": 1}]
    ...
    order_id = order_repository.save_order(
        {
            ...
            "items": order_repository.parse_items_text("PMEZMH, 1"),
        }
    )
    order = order_repository.get_order(order_id)
    assert order and order["items"][0]["article"] == "PMEZMH"
    order_repository.save_order(
        {
            ...
            "items": order_repository.parse_items_text("BPV4MM, 2"),
        }
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Распаковщик исходников СтройМатериалы')
